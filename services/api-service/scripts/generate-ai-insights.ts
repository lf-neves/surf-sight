 ...
 *
 * Usage:
 *   pnpm tsx scripts/generate-ai-insights.ts <spotId>
 *
 * Example:
 *   pnpm tsx scripts/generate-ai-insights.ts 97822015-fe55-4d22-b067-f5ff59caa3ea
 */
...
async function generateAIInsights(spotId: string) {
  try {
    ...
    if (!openAIService.isAvailable()) {
      ...
      logger.info('Current OPENAI_API_KEY value:', process.env.OPENAI_API_KEY ? 'Set (hidden)' : 'Not set');
      process.exit(1);
    }
