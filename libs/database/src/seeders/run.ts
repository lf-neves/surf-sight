...
main().catch((e) => {
  ...
  if (isConnectionError(e)) {
    ...
    console.error(
      '   - For local dev, use a local Postgres (e.g. Docker) and set DATABASE_URL to it.'
    );
    console.error(
      '   - For a remote DB (e.g. RDS), ensure VPN/network access and try DATABASE_CONNECTION_TIMEOUT_MS=30000 in .env'
    );
  }
  process.exit(1);
});
